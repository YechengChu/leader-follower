 ...
 * @author Xin
 */
final class MovingWindowLeader
	extends PlayerImpl
{
  ...
  private double lastAStar;
  private double lastBStar;


	private MovingWindowLeader()
		throws RemoteException, NotBoundException
	{
		super(PlayerType.LEADER, "Leader using MovingWindow data");
	}
...
private float calculatePrice(int p_date)
{
  ...
  int windowSize = 30;
  double t = p_date - 1;
  for(int i = p_date - windowSize; i <= p_date - 1; i++)
  {
  ...
  double m1 = windowSize + lambda;
  double n1 = x_sum;
  double t1 = y_sum + lambda * lastAStar;
  double m2 = x_sum;
  double n2 = x_2_sum + lambda;
  double t2 = x_y_sum + lambda * lastBStar;

  a_star = (t1 * n2 - t2 * n1) / (n2 * m1 - n1 * m2);
  b_star = (t1 * m2 - t2 * m1) / (n1 * m2 - n2 * m1);
	...
	throws RemoteException
{
    ...
    lastAStar = (x_2_sum * y_sum - x_sum * x_y_sum)/(t * x_2_sum - x_sum * x_sum);
    lastBStar = (t * x_y_sum - x_sum * y_sum)/(t * x_2_sum - x_sum * x_sum);
	}
  else
  {
